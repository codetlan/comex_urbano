class PhotosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @photo.update(photo_params)
        ...
        if @publication
          @publication.update(:content => @photo.name + @photo.description + @photo.content + @photo.tag_list.join(' '))
  ...
  def photo_params
    params.require(:photo).permit(:name, :description, :image, :visit, :active, :section_id, :tag_list, :posted_at, :content)
  end
end
